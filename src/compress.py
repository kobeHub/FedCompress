...
def compress_image_model(model, nb_clusters=50, verbose=True):
    ...
    return tf.keras.models.clone_model(
        ...
        clone_function=functools.partial(
            _apply_weight_clustering, clustering_params=clustering_params
        ),
    )
...
def self_compress(
    ...
    ood_data = data_loader(
        batch_size=batch_size, seed=seed, reshape_size=data_shape[1:-1]
    )
    model.trainable = False
    student = model_loader(input_shape=data_shape[1:], num_classes=num_classes)
    student.set_weights(model.get_weights())
    student_model = compress_image_model(
        student, nb_clusters=nb_clusters, verbose=False
    )
    trainer = distiller.Distiller(
        student=student_model, teacher=model, has_labels=False
    )
    trainer.compile(
