...
class SplitDataset:
    ...
    def get_class_stats(labels):
        ...
        class_weights = 1 / compute_class_weight(
    ...
    def get_class_distribution(
        num_classes, num_partitions=10, concentration=0.0, seed=0
    ):

        def _scale(x, low_range=(5e-4, 1e-1), high_range=(50, 1e5)):
            x = 1.0 - x
            ...
            if x <= 0.05:
                ...
                return (
                    math.pow(x / 0.05, 2.0) * 0.05 * (low_range[1] - low_range[0])
                    + low_range[0]
                )
            elif x >= 0.95:
                # Scale values in the top 5% of the range exponentially to high_range
                return (1 - math.pow((1 - x) / 0.05, 2.0)) * 0.95 * (
                    high_range[1] - high_range[0]
                ) + high_range[0]
            else:
        ...
        return np.random.default_rng(seed).dirichlet(
            alpha=np.repeat(_scale(concentration), num_classes), size=num_partitions
        )

    @staticmethod
    def get_label_distribution(
        num_samples, num_partitions=10, concentration=0.0, min_length=1, seed=0
    ):
        ...
        assert (
            min_length >= 0 and min_length * num_partitions <= num_samples
        ), "Invalid value for min_chunk_length"
        assert (
            concentration >= 0 and concentration <= 1.0
        ), "Invalid value for concentration"
        ...
        if concentration == 0:
            ...
            chunk_sizes = scipy.stats.dirichlet.rvs(
                [_scale(concentration)] * num_partitions, size=1
            )[0]
            # Round chunk sizes to the nearest integer and ensure that each chunk
            # has at least min_chunk_length samples
            chunks = [
                max(int(round(size * num_samples)), min_length) for size in chunk_sizes
            ]
            ...
            while sum(chunks) != num_samples:
                ...
                if sum(chunks) != num_samples:
                    while sum(chunks) != num_samples:
                        ...
                        if chunks[chunk_id] + remainder >= min_length:
                            chunks[chunk_id] += remainder
            assert (
                sum(chunks) == num_samples
            ), "Chunks do not sum up to the total number of samples."
            return np.asarray(chunks)
    ...
    def create_partitions(
        ...
        for i, part_class_distibution in enumerate(class_distribution):
            parts[i] = np.around(
                part_class_distibution * class_weights * samples_distribution[i]
            )
        # (Try to) Fix rounding errors.
        if align and (class_samples is not None):
            i = 0
            while not __class__.check_alignment(
                parts, class_samples, samples_distribution, verbose=0
            ):
                if i == max_iter:
                    break
                parts = __class__.align_samples_distribution(
                    parts,
                    class_distribution,
                    class_weights,
                    samples_distribution,
                ...
                parts = __class__.align_class_distribution(
                    parts, class_samples, samples_distribution, min_length, seed + i + 1
                )
                i += 1

        assert __class__.valid_partitions(
            parts, class_samples
        ), "A valid partitioning could not be constructed."

        return parts

    @staticmethod
    def align_class_distribution(
        partitions, class_samples, samples_distribution, min_length=10, seed=0
    ):
        # Fix random seed
        np.random.seed(seed)
        # Local functions
        compute_diff = lambda x: (class_samples - x.sum(axis=0))
        compute_replace_classes = lambda x: (
            True if abs(x) > partitions.shape[0] else False
        )
        ...
        for i, n in enumerate(num_samples_to_replace):
            ...
            indexes = np.random.choice(
                ...
                p=scipy.special.softmax(
                    (samples_distribution - partitions.sum(axis=1))
                ),
            )
    ...
    @staticmethod
    def align_samples_distribution(
        partitions,
        class_distribution,
        class_weights,
        samples_distribution,
        ...
        compute_replace_partitions = lambda x: (
            True if abs(x) > partitions.shape[0] - 1 else False
        )
        compute_replace_classes = lambda x: (
            True if abs(x) > partitions.shape[1] else False
        )
        ...
        for i in range(partitions.shape[0]):
            ...
            part_idxs = np.random.choice(
                ...
                p=__class__.normalize_probabilities(
                    scipy.special.softmax(
                        np.delete(num_samples_to_replace, [i], axis=0)
                    )
                    + 1e-9
                ),
            )
            ...
            for j, c in zip(part_idxs, class_idxs):
                if (
                    partitions[i][c] + np.sign(num_samples_to_replace[i]) > min_length
                ) and (
                    partitions[j][c] - np.sign(num_samples_to_replace[i]) > min_length
                ):
                    # Remove sample for current partition
                    partitions[i][c] = partitions[i][c] + np.sign(
                        num_samples_to_replace[i]
                    )
                    # Move it to another (random) partition
                    partitions[j][c] = partitions[j][c] - np.sign(
                        num_samples_to_replace[i]
                    )
        return partitions
    ...
    def check_alignment(partitions, class_samples, samples_distribution, verbose=0):
        ...
        if ((samples_distribution - partitions.sum(1)).var() == 0.0) and (
            (class_samples - partitions.sum(0)).var() == 0.0
        ):
            if ((samples_distribution - partitions.sum(1)).sum() == 0.0) and (
                (class_samples - partitions.sum(0)).sum() == 0.0
            ):
                return True
        else:
            return False
    ...
    def create_mask(partitions, class_indexes, seed=0):
        ...
        for c_idx in range(len(class_indexes)):
            ...
            for i in range(num_partitions):
                ...
                indexes = np.random.choice(
                    np.array(list(available_class_indexes)),
                    size=num_samples,
                    replace=False,
                )
    ...
    def create_dataset_partition(
        ...
        classes, class_samples, class_indexes, class_weights = (
            __class__.get_class_stats(labels=data[1])
        )
        # Create samples distribution (# samples per partition)
        samples_distribution = __class__.get_label_distribution(
            num_samples=data[1].shape[0],
            num_partitions=num_clients,
            concentration=data_skew,
            min_length=min_samples,
            seed=seed,
        )
        # Create class distribution (% classes per partition)
        class_distribution = __class__.get_class_distribution(
            num_classes=classes.size,
            num_partitions=num_clients,
            concentration=class_skew,
            seed=seed,
        )
        # Create partition arrays (# samples per class for each partition)
        partitions = __class__.create_partitions(
            num_classes=classes.size,
            num_partitions=num_clients,
        ...
        indexes, _ = __class__.create_mask(
            partitions, class_indexes=class_indexes, seed=seed
        )
        # Create mask
    ...
    def split_clients_ids_to_partitions(cid, num_clients, available_ids, seed=0):
        ...
        assert num_clients <= len(
            available_ids
        ), "Number of clients exceeds avaialable clients ids."
        partitions = np.random.choice(
            a=num_clients, size=len(available_ids), replace=True
        )
        return [
            available_ids[i]
            for i in np.argwhere(np.isin(partitions, [int(cid)])).ravel()
        ]
...
class AugmentDataset:
    ...
    def image_ood_prep(x, color_mode="rgb", size=(32, 32)):
        if color_mode == "grayscale":
            return tf.map_fn(__class__.image_train_prep_v2, x)
    ...
    def audio_ood_prep(x, seconds=1, bins=64, sr=16000, to_float=True):
        return __class__.audio_train_prep(
            tf.squeeze(x), seconds=seconds, bins=bins, sr=sr, to_float=to_float
        )
...
class LoadDataset:
    ...
    def get_cifar10(split, with_info=True):
        ...
        info = {
            ...
            "num_examples": (
                test_images.shape[0] if split == "test" else train_images.shape[0]
            ),
        }
        ds = (
            (test_images, test_labels)
            if split == "test"
            else (train_images, train_labels)
        )
    ...
    @staticmethod
    def get_librispeech(
        split=None, with_info=True, reshape_size=None, name="librispeech"
    ):
        with contextlib.redirect_stdout(None):  # Read data from dir
