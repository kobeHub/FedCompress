...
def weighted_average_train_metrics(metrics):
    ...
    results["train_loss"] = sum(
        [num_examples * m["train_loss"] for num_examples, m in metrics]
    ) / sum(train_examples)
    results["train_accuracy"] = sum(
        [num_examples * m["train_accuracy"] for num_examples, m in metrics]
    ) / sum(train_examples)
    # Compute validation metrics
    if all(
        [
            set(["val_loss", "val_accuracy", "num_val_samples"]) <= set(list(keys))
            for keys in metrics_keys
        ]
    ):
        val_examples = [m["num_val_samples"] for _, m in metrics]
        results["val_loss"] = sum(
            [
                num_examples * m["val_loss"]
                for num_examples, (_, m) in zip(val_examples, metrics)
            ]
        ) / sum(val_examples)
        results["val_accuracy"] = sum(
            [
                num_examples * m["val_accuracy"]
                for num_examples, (_, m) in zip(val_examples, metrics)
            ]
        ) / sum(val_examples)
    # Compute model size
    if all("model_size" in keys for keys in metrics_keys):
        ...
        results["models_size"] = sum(results["models_sizes"]) / len(
            results["models_sizes"]
        )
    # Compute validation/ood embedding scores
    if all("embeddings" in keys for keys in metrics_keys):
        results["val_score"] = compute_score(
            np.concatenate([m["embeddings"] for _, m in metrics], axis=0)
        )
...
def set_compression_rnds(compression_step, num_rounds):
    compression_rnds = [
        compression_step * i for i in range(1, (num_rounds + 1) // compression_step)
    ]
    ...
    if num_rounds not in compression_rnds:
        compression_rnds = compression_rnds + [
            num_rounds,
        ]
...
def compression_flag(
    ...
    df["rolling_avg_acc"] = (
        df["val_accuracy"].rolling(window=window, min_periods=1).mean()
    )
    df["rolling_avg_score"] = (
        df["val_score"].rolling(window=window, min_periods=1).mean()
    )
    ...
    if df["mask"].any() and (num_clusters < max_num_clusters):
        df["threshold_acc"] = limit / (
            base * (df["nb_clusters"] - (init_num_clusters))
        ).replace(0, 1)
        df["threshold_score"] = (10 * limit) / (
            base * (df["nb_clusters"] - (init_num_clusters))
        ).replace(0, 1)
        ...
        print(df)
        if ((df["nb_clusters"] == num_clusters).sum() >= patience) and (
            len(df) >= max(init_rounds, 2)
        ):
...
def get_num_clusters(rnd, init_num_clusters, compression_rnds, min_num_clusters=1):
    ...
    upper_closest = lambda r: compression_rnds.index(
        min([i for i in compression_rnds if i >= r], key=lambda x: abs(x - r))
    )
    ...
    if gpus:
        try:
            tf.config.set_logical_device_configuration(
                gpus[0], [tf.config.LogicalDeviceConfiguration(memory_limit=mem)]
            )
        ...
        for f in score_files:
            ...
            for i, j in zip(names, types):
                if i in temp.keys():
                    data[j][rnd(f)].extend(
                        temp[i].numpy()
                        if not isinstance(temp[i], np.ndarray)
                        else temp[i]
                    )
        # Combine
        for i in types:
...
def create_dataloader_fn(ood=False):
    ...
    if ood:
        return {
            "cifar10": data.get_stylegan,
            "spcm": data.get_librispeech,
        }
    return {
